...
sub log_msg 
{
    ...
    {
        syswrite FDLOG, strftime('%d-%m-%Y %H:%M', localtime) . " $s";
        $last_log_line = $s;
    }
    $dump .= $s;
    undef;
...
sub generate_test
{
    my ($pid, $test, $input_fname, $do_copy) = @_;
    ...
    print "> $generate_cmd\n";
    my $tp = $test->{param};
    $tp = '' unless defined $tp;
    execute(
        $generate_cmd,
        { full_name => $fname, name => $name, args => $tp, redir => $redir }
    ) or return undef;
    ...
    for my $t (@$tests)
    {
        ...
        elsif (defined $t->{std_solution_id})
        {
            ...
            execute($run_cmd, {
                full_name => $fname, 
                name => $name, 
                time_limit => $tlimit * 10,
                memory_limit => $mlimit,
                deadline => '-d:' . $tlimit * 10,
            }) or return undef;
...
sub prepare_modules
{
    ...
    for (grep $_->{stype} == $stype, @$problem_sources)
    {
        my (undef, undef, $fname, $name, undef) = split_fname($_->{fname});
        log_msg("module: $fname\n");
        write_to_file("$rundir/$fname", $_->{src})
            or return undef;

        # в данном случае ничего страшного, если compile_cmd нету, 
        # это значит, что модуль компилировать не надо (de_code=1)
        my $compile_cmd = get_cmd('compile', $_->{de_id})
            or next;
...
sub initialize_problem
{
    ...
    $main_source_types{$_} = 1
        for ($cats::generator, $cats::solution, $cats::adv_solution, $cats::checker, $cats::testlib_checker);
...
sub run_checker
{
    ...
    {   
        ...
        $checker_params->{checker_args} =
            $ps->{stype} == $cats::checker ? qq~"$a" "$o" "$i"~ :
            $ps->{stype} == $cats::testlib_checker ? qq~"$i" "$o" "$a"~ :
            die "Bad checker type $ps->{stype}";
...
sub run_single_test
{
    ...
    log_msg("\tUserTime:   $user_time sec\n");
    log_msg("\tMemoryUsed: $memory_used Mb\n");
    log_msg("\tWritten:    $written Mb\n");
...
sub test_solution
{
    ...
    ($problem->{checker_id}) = map $_->{id}, grep
        { $_->{stype} == $cats::checker || $_->{stype} == $cats::testlib_checker } @$problem_sources;
    ...
    if ($compile_cmd ne '')
    {
        ...
        if ($ok)
        {
            my $runfile = apply_params(get_cmd('runfile', $de_id), $problem);
...
sub problem_ready
{
    ...
    $date =~ m/^date:(.*)/ or return 0; #ошибка при интерпретации файла описания задачи
    $date = $1;

    my ($is_uptodate) = $dbh->selectrow_array(qq~
        SELECT 1 FROM problems
        WHERE id = ? AND upload_date <= CATS_TO_EXACT_DATE(?)~, undef,
        $pid, $date);
    $is_uptodate;
}
